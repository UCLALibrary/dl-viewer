...
export default {
  ...
  data() {
    return {
      iiif_manifest_raw: null as null | any,
    }
  },
  computed: {
    firstItemType() {
    ...
    iiif_manifest() {
      if (this.iiif_manifest_raw) {
        return presentation3StrictUpgrade(this.iiif_manifest_raw)
      } else {
        return null
      }
    },
  ...
  async mounted() {
    try {
      ...
      this.iiif_manifest_raw = response.data
    } catch (error) {
